...
export const intro = getVideo(true, "https://www.youtube.com/embed/ujjRcmhJh-I") + `<br>Для определения того, кто является убийцей, вам необходимо исключить из списка подозреваемых все те объекты, которые в явном и неявном виде встретятся в ответах кветста на протяжении трех этапов (в бонусах нет). Например, одна из загадок привела вас к ответу "Петя", исключаем его из списка подозреваемых. Другой пример: ответ на загадку — земноводное, вычеркиваем из списка лягушку. В конце, при всех правильно решенных загадках, у вас должно остаться по одному человеку, месту, орудию. Не все загадки позволяют вычеркивать объекты.`;